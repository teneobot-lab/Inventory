...
export const RejectMasterData: React.FC<RejectMasterDataProps> = ({ 
  ...
  const handleBulkDeleteAction = () => {
    ...
    if (confirm(`Hapus ${selectedIds.size} master barang secara permanen?`)) {
        ...
        else selectedIds.forEach(id => onDeleteItem(id));
        setSelectedIds(new Set());
    }
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Konversi UI Divisor ke Backend Factor (1 / Divisor)
    ...
    const payload = {
      ...
      category: (formData.category || 'UMUM').trim(),
      baseUnit: (formData.baseUnit || 'KG').toUpperCase().trim(),
      ...
      {selectedIds.size > 0 && (
        <div className="bg-slate-900 text-white p-3 rounded-xl flex items-center justify-between shadow-xl animate-in slide-in-from-top-4 sticky top-4 z-40 border border-white/10 ring-4 ring-blue-500/10">
            ...
            <div className="flex gap-2">
                <button onClick={() => setSelectedIds(new Set())} className="px-4 py-1.5 text-[9px] font-black uppercase tracking-widest hover:bg-white/10 rounded-lg">Batal</button>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-xl p-8 animate-in zoom-in duration-200 border dark:border-slate-800 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
               <div className="space-y-1">
                 ...
                 <input required className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-500 rounded-xl p-4 outline-none font-bold text-base dark:text-white uppercase transition-all" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
               </div>
               
               <div className="grid grid-cols-2 gap-4">
                 <div className="space-y-1">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">SKU</label>
                   <input required className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-500 rounded-xl p-4 outline-none font-mono font-bold text-sm dark:text-white uppercase" value={formData.sku} onChange={e => setFormData({...formData, sku: e.target.value})} />
                 </div>
                 <div className="space-y-1">
                   <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Kategori</label>
                   <input required className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-500 rounded-xl p-4 outline-none font-bold text-sm dark:text-white uppercase" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})} />
                 </div>
               </div>
               ...
               <div className="p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 space-y-4 shadow-inner">
                  <div className="flex justify-between items-center">
                    ...
                    <button type="button" onClick={addConversion} className="text-white text-[9px] font-black bg-blue-600 px-4 py-2 rounded-lg hover:bg-blue-700 uppercase tracking-widest">Tambah Varian</button>
                  </div>
                      ...
                      {uiConversions.map((c, idx) => (
                        <div key={idx} className="flex items-center gap-3 bg-white dark:bg-slate-950 p-3 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
                           <div className="flex items-center gap-2 flex-1">
                               ...
                               <input placeholder="Unit" className="flex-1 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-400 rounded-lg p-2 text-xs text-center font-black uppercase outline-none" value={c.name} onChange={e => updateConversion(idx, 'name', e.target.value.toUpperCase())} required />
                           </div>
                           ...
                           <div className="flex-1 relative">
                               ...
                               <input type="number" step="any" className="w-full bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-blue-400 rounded-lg p-2 text-xs text-center font-black outline-none" value={c.divisor} onChange={e => updateConversion(idx, 'divisor', parseFloat(e.target.value))} required />
                           </div>

                           <div className="flex items-center gap-2">
                               <span className="text-slate-400 font-bold text-[9px] uppercase w-8 truncate">{formData.baseUnit}</span>
                               <button type="button" onClick={() => removeConversion(idx)} className="text-slate-200 hover:text-red-500 transition-all"><Trash2 size={18}/></button>
                           </div>
                        </div>
                      ))}
               ...
               <div className="flex gap-3 pt-4">
                    <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 py-4 text-slate-400 font-black uppercase tracking-widest text-[10px] hover:bg-slate-100 rounded-xl">Batal</button>
                    <button type="submit" className="flex-[2] bg-blue-600 text-white py-4 rounded-xl font-black shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all uppercase tracking-[0.2em] text-xs flex justify-center items-center gap-3">
                        <Save size={20} strokeWidth={2.5} /> Simpan Data Master
                    </button>
               </div>
            </form>
          </div>
