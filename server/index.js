...
app.get('/api/health', (req, res) => sendRes(res, 200, true, "System Auditor: Online"));
...
app.post('/api/inventory', async (req, res) => {
    ...
    try {
        const sql = `INSERT INTO inventory (id, name, sku, category, stock, min_stock, unit, conversions, price, status, last_updated) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
                     ON DUPLICATE KEY UPDATE name=VALUES(name), sku=VALUES(sku), category=VALUES(category), min_stock=VALUES(min_stock), unit=VALUES(unit), conversions=VALUES(conversions), price=VALUES(price), status=VALUES(status), last_updated=VALUES(last_updated)`;
...
app.delete('/api/inventory/:id', async (req, res) => {
    try {
        ...
        if (usage[0].count > 0) {
            ...
            sendRes(res, 200, true, "Soft Delete: Item marked as Inactive due to transaction history.");
        } else {
            await pool.query('DELETE FROM inventory WHERE id = ?', [req.params.id]);
            sendRes(res, 200, true, "Hard Delete: Item removed permanently.");
        }
    } catch (err) { sendRes(res, 500, false, err.message); }
});
...
app.post('/api/transactions', async (req, res) => {
    ...
    try {
        ...
        await conn.rollback(); 
        sendRes(res, 400, false, "Transaction Failed: " + err.message); 
    } finally { conn.release(); }
});

// BULK IMPORT (TRANSACTIONAL)
app.post('/api/inventory/bulk', async (req, res) => {
    ...
    try {
        ...
        for (const i of items) {
            const sql = `INSERT INTO inventory (id, name, sku, category, stock, min_stock, unit, conversions, price, status, last_updated) 
                         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
                         ON DUPLICATE KEY UPDATE name=VALUES(name), category=VALUES(category), stock=stock+VALUES(stock), price=VALUES(price), last_updated=NOW()`;
            await conn.query(sql, [i.id, i.name, i.sku, i.category, i.stock || 0, i.minStock || 0, i.unit, JSON.stringify(i.conversions || []), i.price || 0, 'active', new Date()]);
        }
        await conn.commit();
        sendRes(res, 200, true, "Bulk Import Successful");
    } catch (err) { 
        await conn.rollback(); 
        sendRes(res, 500, false, "Bulk Import Failed: " + err.message); 
    } finally { conn.release(); }
});

// REJECT MODULE (STANDALONE)
...
app.post('/api/reject/transactions', async (req, res) => {
    const tx = req.body;
    try {
        // Standalone: Does NOT touch 'inventory' table stock
        ...
        sendRes(res, 201, true, "Reject Recorded (Standalone)");
    } catch (err) { sendRes(res, 500, false, err.message); }
});

app.listen(PORT, '0.0.0.0', () => console.log(`ERP AUDITOR: Backend Ready on ${PORT}`));
